/*	User script for collecting analytics from blocks, sending that data on to a Matomo
	server (https://matomo.org) for later analysis. Credentials and other settings must be
	defined in a configuration file at script/files/MetricsProviderSettings.json.
	If not found, this script will generate an example configuration file you then need to
	tweak to make things work with your system.

	IMPORTANT: Do not install this user script unless you have a working Matomo server
	instance at your disposal. After first run, modify the script's configuration file
	with your settings then restart Blocks to connect to Matomo.
...
export class MetricsProvider extends Script {
	...
	/*	Look for a settings file. If not present we make one with default
		settings to use as an example.
	*/
	private readSettingsFromFile() {
		...
		}).catch(error => {	// Failed reading file. Try wo write example file.
	...
	/** Finds all Display Spots in the system and start tracking them.
	 */
...
class TrackedSpot {
	...
	private onNavigation(sender: DisplaySpot, message: { foundPath: string }): void { // (sender.fullName ? sender.fullName + "/" : "")
		log("Got some navigation data " + message.foundPath)
		let trackedPath = settings.TRACKED_URL + "/" + sender.fullName + "/" + this.trackedSpot.playingBlock + "/" + (this.currentLanguage ? this.currentLanguage : "") + replaceDoubleSlashWSingle(
			message.foundPath)
		//If we have a language selected we syntesize it into the full path.
